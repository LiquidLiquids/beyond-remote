...
function createRemote(url, init, options) {
	...
	var remote = function remote() {
		_this.trigger('start');
		var result = Promise.race([fetch(url, init), timeoutPromise(options.timeout)]).then(function (response) {
			var isSuccess = response.ok || response.status >= 200 && response.status < 300;
			if (isSuccess) {
				_this.trigger('success', response.clone());
			} else {
				throw response;
			}
			_this.trigger('complete', response.clone());
			return response.headers.get('content-type') && response.headers.get('content-type').indexOf('json') >= 0 ? response.json() : response;
		})['catch'](function (response) {
			_this.trigger('error', response.clone ? response.clone() : response);
			_this.trigger('complete', response.clone ? response.clone() : response);
			throw response;
		});
		_this.trigger('send');
		return result;
...
Remote.prototype.create = function (opts) {
	...
	var url = typeof opts.url === 'string' || opts.url == null ? mergeUrl(opts.basePath, opts.url || '') : opts.url;
	// console.log(url)
	for (var k in opts) {
	...
	if (opts.headers) {
		var headers = {};
		for (var _k in opts.headers) {
			headers[_k.toLowerCase()] = init.headers[_k];
		}
